...
| Submit a card rating (write `card_study_state`) | Direct PostgREST call via repository, computed by the `domain/study` module client-side   | The card-selection _math_ runs in the client (pure, testable function — see [Architecture](04-architecture.md)); only the _result_ is written to the database. No server-side computation needed since there's nothing security-sensitive about a user updating their own card's weight.                                                                                                                                                                                       |
